// ============================================================================
// model1_persistent_h64.cu —— MODEL1 模型 + 64 头的模板实例化文件
// ============================================================================
//
// 【这个文件解决什么实际问题？】
//
...
// MODEL_TYPE=MODEL1、NUM_HEADS=64 这组参数生成一份真正的函数"，这个函数
// 才会被编译成 GPU 上能跑的机器码。这一行 `template void ...<MODEL1, 64>(...)`
...
//   - model1_persistent_h64.cu : MODEL1 + 64 头  ← 本文件
...
//
//   维度差异：
//     - V3.2 : HEAD_DIM_K=576（512 nope + 64 rope），HEAD_DIM_V=512
//     - MODEL1: HEAD_DIM_K=512（448 nope + 64 rope），HEAD_DIM_V=512
//
//   Scale（反量化系数字段）差异：
//     - V3.2 : 4 个 fp32 scale，每 128 维一组，紧跟在 fp8 K 数据后面
//     - MODEL1: 8 个 fp8_e8m0 scale，每 64 维一组，存在 page block 末尾的独立区域
//
//   额外功能（MODEL1 独有）：
//     - 动态 topk_length：每个 batch 可以有不同的 topk 长度
//     - extra KV cache：额外的索引池，存放原始 KV cache 之外的补充 token
//
// 所以 MODEL1 的 kernel 代码里有 `if constexpr (MODEL_TYPE == ModelType::MODEL1)`
// 这种分支，编译时只有 MODEL1 那条分支会被保留下来。
//
// 【为什么文件名带 "persistent"】
//
...
// 模板显式实例化：请编译器用 <MODEL1, 64> 这组参数生成一份函数代码
//   - ModelType::MODEL1：另一种模型架构，HEAD_DIM_K=512（448 nope + 64 rope），
//     支持动态 topk_length 和 extra KV cache（详见 splitkv_mla.cuh 的注释）
//   - 64：NUM_HEADS=64，Q 有 64 个注意力头
// 编译后这个文件会提供一份 run_flash_splitkv_mla_fp8_sparse_kernel<MODEL1, 64>
